...
def defineElongRateRegions(input_df_basename, df, dna_sequence, min_length, max_length, min_rate, max_rate, pause_rate, pause_chance):
    ...
    if strand == '+':
        ...
        regions_df["absolute_start"] = regions_df["absolute_end"] - 1 - regions_df["region_end_coord"]
        regions_df["absolute_end"] = regions_df["absolute_end"] - 1 - regions_df["region_start_coord"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pause_elong_rate', type=float, default=1, help='elongation rate (in nt/min) at simulated PolII pause sites', required=False)
    ...
    base_filename = os.path.splitext(os.path.basename(args.tsv))[0]
